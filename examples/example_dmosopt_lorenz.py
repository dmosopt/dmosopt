...
def get_lorenz_target(sim_t0=0., sim_tmax=100., tar_t0=20., tar_tmax=100., tar_dt=0.1, p=(10, 28, 8/3)):
    ''' p = (sigma, rho, beta)
    ...
    target_soln = solve_ivp(lorenz, (sim_t0, sim_tmax), X0, args=p, method='Radau',
...
#    param_values = np.asarray([pp[k] for k in sorted(pp)])
    ...
    eval_soln = solve_ivp(lorenz, (t0, tmax), X0, args=param_values, method='Radau',
                  dense_output=True, vectorized=True) 
...
if __name__ == '__main__':
    ...
    space = {'s': [5., 15], 'r':[15, 35], 'b':[1,10]}
    problem_parameters = {}
    objective_names = ['x', 'y', 'z']
    
    # Create an optimizer
    dmosopt_params = {'opt_id': 'dmosopt_lorenz',
