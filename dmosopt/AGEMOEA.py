...
def optimization(
    ...
    for i in it:
        ...
        pool_idxs = tournament_selection(
            local_random, pop, poolsize, toursize, rank, -crowd_dist
        )
    ...
    sorted_population = np.lexsort(
        tuple((metric for metric in [rank, -crowd_dist])), axis=0
    )
...
def environmental_selection(
    local_random, population_parm, population_obj, pop, nInput, nOutput, logger=None
):
    ...
    if count < pop:
        ...
        selected[
            front_1[local_random.choice(selection_rank, size=pop, replace=False)]
        ] = True
