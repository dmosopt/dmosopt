    ...
    def __init__(self, xin, yin, nInput, nOutput, xlb, xub, seed=None, gp_lengthscale_bounds=None, gp_likelihood_sigma=None, adam_lr=0.01, n_iter=5000, min_loss_pct_change=0.1, cuda=False, logger=None):
        ...
        if logger is not None:
            ...
            for i in range(nOutput):
                logger.info(f"MEGP_Matern: y_{i+1} range is {(np.min(yin[:,i]), np.max(yin[:,i]))}")
...
class CRV_Matern:
    ...
    def __init__(self, xin, yin, nInput, nOutput, xlb, xub, seed=None, batch_size=50, inducing_fraction=0.2, min_inducing=100, gp_lengthscale_bounds=(1e-6, 100.0), gp_likelihood_sigma=1.0e-4, natgrad_gamma=0.1, adam_lr=0.01, n_iter=30000, min_elbo_pct_change=0.1, num_latent_gps=None, logger=None):
        ...
        if logger is not None:
            ...
            for i in range(nOutput):
                logger.info(f"CRV_Matern: y_{i+1} range is {(np.min(yin[:,i]), np.max(yin[:,i]))}")
        ...
        for kernel in gp_model.kernel.kernels:
            kernel.lengthscales = bounded_parameter(np.asarray([gp_lengthscale_bounds[0]]*nInput, dtype=np.float64),
        ...
        for it in range(iterations):
            ...
            if it >= 2000:
                ...
                if (it % 1000 == 0):
                    logger.info(f"CRV_Matern: iteration {it} mean elbo pct change: {mean_elbo_pct_change:.04f}")
                if mean_elbo_pct_change < min_elbo_pct_change:
                    logger.info(f"CRV_Matern: likelihood change at iteration {it+1} is less than {min_elbo_pct_change} percent")
                    break
...
class SVGP_Matern:
    def __init__(self, xin, yin, nInput, nOutput, xlb, xub, seed=None, batch_size=50, inducing_fraction=0.2, min_inducing=100, gp_lengthscale_bounds=(1e-6, 100.0), gp_likelihood_sigma=1.0e-4, natgrad_gamma=0.1, adam_lr=0.01, n_iter=30000, min_elbo_pct_change=1.0, logger=None):
        if not _has_gpflow:
