...
class MDSPP_Matern:
    def __init__(
        ...
        def train(
            ...
            mll = gpytorch.mlls.DeepPredictiveLogLikelihood(
                gp_likelihood, gp_model, num_data=train_y.size(0), beta=0.05
            )
            loss_log = []
    ...
    def predict(self, xin):
        ...
        with ExitStack() as stack:
            ...
            in_dataset = TensorDataset(x)
            in_loader = DataLoader(in_dataset, batch_size=batch_size, shuffle=False)
            means, vars = self.sm.predict(x)

            weights = self.sm.quad_weights.unsqueeze(-1).exp().cpu()
            mean = (weights * means).sum(0)
            var = (
                (weights * vars).sum(0)
                + (weights * means**2).sum(0)
                - (weights * means).sum(0) ** 2
            )
