...
class SA_FAST:
    ...
    def __init__(self, lo_bounds, hi_bounds, param_names, output_names, logger=None):
        ...
        self.problem = { 'num_vars': len(param_names),
                         'names': param_names,
                         'bounds': bounds }
    ...
    def analyze(self, model, num_samples=10000):
        ...
        Sis = list([fast.analyze(self.problem, Y[:,i], print_to_console=False)
...
class SA_DGSM:
    ...
    def analyze(self, model, num_samples=10000):
        ...
        Sis = list([dgsm.analyze(self.problem, param_values, Y[:,i], print_to_console=False)
                    for i in range(Y.shape[1])])
