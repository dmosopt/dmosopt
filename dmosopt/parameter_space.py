...
Based on code from pygpso: https://github.com/jajcayn/pygpso
"""
...
class LeafNode(NodeMixin):
    ...
    def __init__(
        ...
        assert all(isinstance(param_name, str) for param_name in parameter_names)
    ...
    def grow(self, depth):
        ...
        for _ in range(depth):
            ...
            normed_coords.append(
                np.array([leaf.get_center_as_list(normed=True) for leaf in leaves])
            )
    ...
    def ternary_split(self):
        ...
        children_leaves = [
            LeafNode(
                norm_bounds=self._replace_normed_coord(largest_dim, splitted_coord),
                scaler=self.scaler,
            ...
            for splitted_coord, idx in zip(pairwise(children_coords), ["l", "c", "r"])
        ]
...
class ParameterSpace(LeafNode):
    ...
    def __init__(self, parameter_bounds, parameter_names):
        ...
        norm_bounds_full = [NORM_PARAMS_BOUNDS for _ in range(len(parameter_bounds))]
