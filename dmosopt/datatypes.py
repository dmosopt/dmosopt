...
EpochResults = namedtuple("EpochResults", ["best_x", "best_y", "gen_index", "x", "y"])
...
class StateMachine:
    ...
    def run(self, args):
        ...
        if not self.end_states:
            raise RuntimeError(
                "StateMachine.run: at least one state must be an end_state"
            )
        ...
        try:
            ...
            raise RuntimeError(
                "StateMachine.run: error in operation for state {self.state}"
            )
